...
export default function ArtigoPage() {
  ...
  // Autoras da equipe apontam para /equipe; vozes convidadas, para o Quarto de Hóspedes.
  const autoresComPagina = artigo.autores
    .map((autor) => ({
      autor,
      href: autor.equipeId
        ? `/equipe/${autor.equipeId}`
        : autor.hospedeId
        ? `/comodos/quarto-de-hospedes/${autor.hospedeId}`
        : null,
    }))
    .filter((item): item is { autor: typeof item.autor; href: string } => item.href !== null);
        ...
        >
          ...
          {autoresComPagina.length > 0 && (
            <div className="mt-2 flex flex-wrap items-center justify-center gap-3">
              {autoresComPagina.map(({ autor, href }) => (
                <Link
                  key={href}
                  href={href}
